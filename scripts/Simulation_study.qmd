---
title: "Pilot study — convenience sampling bias (confirmatory)"
author: ""
date: today
format:
  html:
...
We follow Supplement S1 and represent convenience sampling with a selection indicator $S$, where $S = 1$ if an observation is retained. The **selection process** shapes which values of $X$ enter the convenience sample ($S \to X$). The structural model for $Y$ remains $X \to Y$.
...
%%| fig-cap: "Selection DAG: S influences the observed distribution of X; X influences Y."
flowchart LR
  S["S (selection)"] --> X["X (continuous predictor)"]
  X --> Y["Y (continuous outcome)"]
```
...
dgp <- function(
  ...
  sample_selected <- function(score_fun, n) {
    ...
    out <- list()
    n_got <- 0L
    batch <- as.integer(max(n, ceiling(n * pool_mult / 10)))
    guard <- 0L
    while (n_got < n && guard < 200L) {
      guard <- guard + 1L
      pop <- draw_population(batch)
      p <- plogis(alpha + gamma * score_fun(pop))
      keep <- runif(nrow(pop)) < p
      sel <- pop[keep, , drop = FALSE]
      if (nrow(sel) > 0L) {
        out[[length(out) + 1L]] <- sel
        n_got <- n_got + nrow(sel)
      }
    }
    if (n_got < n) {
      stop(
    ...
    do.call(rbind, out)[seq_len(n), , drop = FALSE]
  }
...
# Scenario 1: probabilistic (logistic) restriction
checks(niter, beta1, restrict, n, formula, dgp)
sim <- run_simulations(niter, beta1, restrict, n, formula, dgp)

# Scenario 2: deterministic (sinusoidal) restriction
checks(niter, beta1, restrict, n, formula, dgp_det)
sim_det <- run_simulations(niter, beta1, restrict, n, formula, dgp_det)
```
...
file.rename(
  ...
  "Mean relative bias restricted on X (logistic).png"
)
file.rename(
  "Mean relative bias for convenience sample restricted on Y.png",
  "Mean relative bias restricted on Y (logistic).png"
)
file.rename("Power Plot.png", "Power Plot (logistic).png")
```

##### Deterministic (sinusoidal) selection

```{r render-summary-deterministic}
simulation_results(sim_det, restrict)
power(sim_det)
file.rename(
  "Mean relative bias for convenience sample restricted on X.png",
  "Mean relative bias restricted on X (sinusoidal).png"
)
file.rename(
  "Mean relative bias for convenience sample restricted on Y.png",
  "Mean relative bias restricted on Y (sinusoidal).png"
)
file.rename("Power Plot.png", "Power Plot (sinusoidal).png")
```
...
We explore four confirmatory violations below. Where feasible we keep the Helper.R API
(`dgp_*(beta1, restrict, n)` returning `convenience_x` / `convenience_y`); some scenarios
(negative binomial mean–variance, multilevel / EIV) require custom summaries.
...
Here a latent common cause $Z$ drives both the predictor and the outcome (fork $X \leftarrow Z \rightarrow Y$), while the dashed path $X \dashrightarrow Y$ is the target association. Selection $S$ that depends on $Z$ (e.g. hospital / clinic recruitment) acts as a **confounder pathway** into the observed sample: restricting on $S$ changes the distribution of $Z$ and thereby biases the naive $X$–$Y$ association if $Z$ is omitted from the analysis model.

```{mermaid}
%%| label: fig-confounder
%%| fig-cap: "Confounding fork: Z is a common cause of X and Y; S depends on Z (selection into the study)."
flowchart TB
  Z["Z (latent common cause)"] --> X["X"]
  Z --> Y["Y"]
...
dgp_confounder <- function(
  ...
  draw_population <- function(n) {
    z <- rnorm(n)
    x <- 0.7 * z + rnorm(n, sd = 0.7)
    y <- beta0 + beta1 * x + beta_z * z + rnorm(n, sd = sigma)
    data.frame(x = x, y = y, z = z)
  }

  sample_on_z <- function(n) {
    if (restrict == "none") {
    ...
    while (n_got < n && guard < 200L) {
      guard <- guard + 1L
      pop <- draw_population(batch)
      keep <- runif(nrow(pop)) < plogis(alpha + gamma * pop$z)
      sel <- pop[keep, c("x", "y"), drop = FALSE]
      if (nrow(sel) > 0L) {
        out[[length(out) + 1L]] <- sel
        n_got <- n_got + nrow(sel)
  ...
  # both samples: selection on Z (confounder), not on X or Y alone
  df <- sample_on_z(n)
  list(convenience_x = df, convenience_y = df)
}
```

### iii) Nonlinear mean–variance relationship (negative binomial)
...
where the negative-binomial variance is

$$
\operatorname{Var}(Y \mid X) = \mu(X) + \frac{\mu(X)^2}{\theta}.
$$

As $\theta \to \infty$ one recovers Poisson equidispersion $\operatorname{Var} = \mu$; finite $\theta$ induces overdispersion (mean–variance coupling). Range restriction on $X$ or $Y$ then interacts with this nonlinear link and heteroscedasticity, so OLS slopes are misspecified estimands.
...
### iv) Mixed / multilevel model with group-level selection (and EIV note)

**Mixed / multilevel models** address dependence and heterogeneity. For observations $i$ in groups $j$ (e.g. patients in hospitals),
...
**Errors-in-variables (EIV)** is a different problem: the true covariate $X^*$ is latent and one observes $W = X^* + U$, so naive regression of $Y$ on $W$ attenuates

$$
\beta_{\mathrm{naive}} = \beta \frac{\sigma_X^2}{\sigma_X^2 + \sigma_U^2}.
$$

Mixed models and EIV both introduce latent variables, but for different reasons (group heterogeneity vs measurement error). They may be combined; here we focus on **selection on groups**: $S$ retains hospitals with large $|b_j|$ (or a logistic function of $b_j$), so convenience samples over-represent extreme cluster effects.

```{mermaid}
%%| label: fig-mixed-selection
%%| fig-cap: "Multilevel DGP with group-level selection: S depends on the hospital random effect b_j."
flowchart TB
  Bj["b_j (hospital RE)"] --> Y["Y_ij"]
  X["X_ij"] --> Y
  E["ε_ij"] --> Y
  Bj --> S["S (select hospital)"]
```
...
dgp_mixed <- function(
  ...
  draw_selected <- function(n) {
    ...
    if (restrict == "none") {
      ...
      if (length(keep_g) < 2L) {
        keep_g <- order(abs(b), decreasing = TRUE)[seq_len(max(2L, n_groups %/% 4))]
      }
    }
    rows <- list()
    for (j in keep_g) {
...
```{r exploratory-run}
# Optional Monte Carlo for exploratory DGPs (same Helper API where compatible).
# Uncomment to run; keep niter modest while prototyping.
#
# checks(niter, beta1, restrict, n, formula, dgp_collider)
# sim_col <- run_simulations(niter, beta1, restrict, n, formula, dgp_collider)
# simulation_results(sim_col, restrict); power(sim_col)
#
# checks(niter, beta1, restrict, n, formula, dgp_confounder)
# sim_cnf <- run_simulations(niter, beta1, restrict, n, formula, dgp_confounder)
#
# checks(niter, beta1, restrict, n, formula, dgp_negbin)
# sim_nb <- run_simulations(niter, beta1, restrict, n, formula, dgp_negbin)
#
# checks(niter, beta1, restrict, n, formula, dgp_mixed)
# sim_mix <- run_simulations(niter, beta1, restrict, n, formula, dgp_mixed)
```
