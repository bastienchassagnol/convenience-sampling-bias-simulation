...
Without a thin wrapper, each Monte Carlo scenario would repeat the same four confirmatory steps (`checks()`, `run_simulations()`, `simulation_results()`, `power()`) plus renaming of the fixed PNG filenames that `Helper.R` writes. @lst-run-scenario packages that pipeline so every DGP only needs its function and a figure tag, while still receiving all design parameters as explicit arguments (no reliance on the calling environment).
...
We explore four confirmatory violations below. Each scenario keeps the Helper.R API
(`dgp_*(beta1, restrict, n)` returning `convenience_x` / `convenience_y`) and reports
mean relative bias and power via `simulation_results()` and `power()`.
...
<!-- Temporarily disabled: linear mixed / hierarchical exploratory section

### iv) Linear mixed model with group-level selection
