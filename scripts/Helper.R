...
validate_dgp_result <- function(res, beta1, restrict, n, formula, require_binary_for = NULL,
                                min_random_n = 1000, min_var = 1e-8, min_unique_y_numeric_x = 4) {

  # Global structure checks
  loc <- sprintf("[beta1=%s, restrict=%s]", as.character(beta1), as.character(restrict))
  if (!is.list(res)) stop("dgp() must return a list ", loc)
  expected <- c("convenience_x", "convenience_y")
  if (!setequal(names(res), expected) || length(res) != length(expected)) {
    stop("dgp() must return a named list with exactly: ",
         paste(expected, collapse = ", "), " ", loc)
  }
  
  # Per-dataset structural checks
  for (nm in expected) {
    df <- res[[nm]]
    if (!is.data.frame(df)) stop(sprintf("dgp()[[%s]] must be a data.frame %s", nm, loc))
    ...
    if (length(missing_vars) > 0) {
      stop(sprintf("The formula references variable(s) not present in `data`: %s",
                   paste(missing_vars, collapse = ", ")))
    }
    
    if (!("y" %in% names(df))) stop(sprintf("dgp()[[%s]] is missing column: y %s", nm, loc))
    if (!("x" %in% names(df))) stop(sprintf("dgp()[[%s]] is missing column: x %s", nm, loc))
    if (!is.numeric(df$y)) stop(sprintf("`y` must be numeric in dgp()[[%s]] %s", nm, loc))
    if (all(is.na(df$y))) stop("`y` contains only missing values.")
    
    # check minimum variance in x and y
    vy <- .var_non_na(df$y)
    if (is.na(vy) || vy <= min_var) {
      stop(sprintf("`y` variance too small (var=%.3g <= %.3g) in dgp()[[%s]] %s",
                   vy, min_var, nm, loc))
    }
    
    vx <- .var_non_na(df$x)
    if (is.na(vx) || vx <= min_var) {
      stop(sprintf("`x` variance too small (var=%.3g <= %.3g) in dgp()[[%s]] %s",
                   vx, min_var, nm, loc))
    }
    ...
    if (uy < 4L) {
      stop(sprintf("`y` must have at least 4 unique finite values (got %d) in dgp()[[%s]] %s",
                   uy, nm, loc))
    }
    ...
    if (identical(x_type, "numeric")) {
      ...
      if (ux < 4L) {
        stop(sprintf("When `x` is numeric, `x` must have at least 4 unique finite values (got %d) in dgp()[[%s]] %s",
                     ux, nm, loc))
      }
    }
  }
  ...
  if (identical(restrict, "none")) {
    if (!isTRUE(all.equal(res$convenience_x, res$convenience_y))) {
      stop("When restrict = 'none', `convenience_x` and `convenience_y` must be identical ", loc)
    }
  }
...
classify_x <- function(x) {
  ...
  if (is.factor(x) || is.ordered(x)) {
    ...
    stop(sprintf("`x` is a factor with %d levels; only 2-level factors are allowed (binary).", k))
  }
  ...
  stop(sprintf("`x` must be numeric, logical, or a factor/ordered factor with 2 levels. Got class: %s",
...
check_x_y_types <- function(df) {
  ...
  if (identical(x_type, "numeric")) {
    ...
    if (length(vals) <= 2) {
      stop("`x` was classified as numeric but has ≤ 2 distinct values; this is effectively binary.")
    }
  }
  x_type
}
...
mc_parametric_random <- function(beta_true, n, n_match, formula, B,
                                 dgp_fun) {
  ...
  for (b in seq_len(B)) {
    ...
    # sample to match n
    idx <- if (nrow(df) >= n_match) sample.int(nrow(df), n_match, replace = FALSE)
    else                      sample.int(nrow(df), n_match, replace = TRUE)
    mf  <- model.frame(f, data = df[idx, , drop = FALSE], na.action = na.omit)
    if (nrow(mf) < 2L) next
    fit <- try(lm(f, data = mf), silent = TRUE); if (inherits(fit, "try-error")) next
    cf  <- coef(fit)
    # estimability check
    if (anyNA(cf)) next
...
check_random_unbiased_at_n_parametric <- function(beta_true, n, n_match, formula, B,
                                                  dgp_fun, level = 0.9,
                                                  abs_tol, rel_tol,
                                                  min_ok = 100) {
  mc <- mc_parametric_random(beta_true, n, n_match, formula, B, dgp_fun)
  if (length(mc) < min_ok) {
    stop(sprintf("Too few MC estimates (got %d). Increase B or inspect DGP.", length(mc)))
  }
  mc_mean <- mean(mc)
  ...
  if (!is.null(abs_tol) || !is.null(rel_tol)) {
    thr <- max(if (is.null(abs_tol)) 0 else abs_tol,
...
checks <- function(niter, beta1, restrict, n, formula, dgp_fun,
                   # MC preflight knobs
                   B = 200L, level = 0.90, abs_tol = 0.02, rel_tol = 0.05,
                   return_diag = FALSE) {
  ...
  if (!is.character(restrict) || length(restrict) != 4 || !identical(restrict, allowed_restrict))
    stop("`restrict` must be exactly c('none','weak','moderate','strong') in this order.")
  ...
  rhs_ok <- identical(as.character(formula[[3]]), ".") || ("x" %in% all.vars(formula[[3]]))
  if (!rhs_ok)
    stop("The right-hand side of `formula` must be `.` or contain predictor `x` (e.g., y ~ x + z).")
  ...
  for (b1 in beta1) {
    for (rr in restrict) {
      ...
      res <- tryCatch(dgp_fun(beta1 = b1, restrict = rr, n = n),
                      error = function(e) e)
      ...
      if (inherits(res, "error")) {
        ...
        warnf("dgp() error: failed for beta1=%s, restrict=%s: %s",
              as.character(b1), rr, conditionMessage(res))
        next
      }
      ...
      v_ok <- TRUE
      tryCatch(
        validate_dgp_result(res, beta1 = b1, restrict = rr, n=n, formula = formula),
        error = function(e) {
          v_ok <<- FALSE
          warnf("dgp() error: failed for beta1=%s, restrict=%s: %s",
                as.character(b1), rr, conditionMessage(e))
        }
      )
      
      # if validation failed, record rows with the error for both samples
      for (nm in c("convenience_x","convenience_y")) {
        n_match <- tryCatch(nrow(res[[nm]]),
                            error = function(e) NA_integer_)
        
        f <- as.formula(formula)
        mf0 <- tryCatch(model.frame(f, data = res[[nm]], na.action = na.omit),
                        error = function(e) NULL)
        
        if (is.null(mf0) || nrow(mf0) < 2L) {
          warnf("Model frame too small/failed for beta1=%s, restrict=%s, sample=%s",
                as.character(b1), rr, nm)
        } else {
          fit0 <- try(lm(f, data = mf0), silent = TRUE)
          if (inherits(fit0, "try-error") || anyNA(coef(fit0))) {
            warnf("Non-estimable model (error or NA coefficients) for beta1=%s, restrict=%s, sample=%s",
                  as.character(b1), rr, nm)
          }
        }
        ...
        if (inherits(chk, "error")) {
          ...
          diag[[row_id]] <- data.frame(
            ...
            B         = B,
            ok        = FALSE,
            error_msg = sprintf("check_random_unbiased_at_n_parametric() error: %s",
                                conditionMessage(chk)),
            stringsAsFactors = FALSE
          )
          warnf("Check errored for beta1=%s, restrict=%s, sample=%s (n=%s): %s",
                as.character(b1), rr, nm, as.character(n_match), conditionMessage(chk))
          next
        }
        ...
        diag[[row_id]] <- data.frame(
          ...
          B         = chk$B,
          ok        = chk$ok,
          error_msg = if (!chk$ok) sprintf(
            paste0("Biased estimate in unrestricted sample: MC mean=%.4g, %d%% MC=[%.4g, %.4g], ",
                   "beta1=%.4g; appears biased at this n (by CI/tolerance)."),
            chk$mc_mean, as.integer(100*level), chk$ci[1], chk$ci[2], b1) else NA_character_,
          stringsAsFactors = FALSE
        )
        
        if (!chk$ok) {
          warnf(paste0("Biased estimate in unrestricted sample: for beta1=%s, restrict=%s, sample=%s (n=%d): ",
                       "MC mean=%.4g, %d%% MC=[%.4g, %.4g], beta1=%.4g. ",
                       "Random baseline appears biased at this n (by CI and tolerance rules)."),
                as.character(b1), rr, nm, n_match,
                chk$mc_mean, as.integer(100*level), chk$ci[1], chk$ci[2], b1)
        }
      }
    }
  }
  ...
  if (all_ok) {
    message("Well done! All checks passed; you're good to go with the simulation.")
  }
  
  if (isTRUE(return_diag)) return(diag_df)
  invisible(diag_df)
}
...
cohens_d_ci <- function(x, y, conf.level = 0.95, na.rm = TRUE) {
  ...
  if (n1 < 2 || n2 < 2) {
    return(data.frame(d = NA_real_, se = NA_real_,
...
one_simulation_run <- function(beta1, restrict, n, formula, dgp_fun){
  ...
  make_na_params <- function(x_type = "numeric") {
    base <- c(coef = NA_real_, se = NA_real_, R2 = NA_real_,
              coef_std = NA_real_, se_std = NA_real_, R2_std = NA_real_)
    if (identical(x_type, "numeric")) {
      c(base,
        pearson = NA_real_,
  ...
  data <- lapply(seq_len(nrow(grid)), function(i) {
    b1 <- grid$beta1[i]
    rr <- grid$restrict[i]
    res <- tryCatch(dgp_fun(beta1 = b1, restrict = rr, n=n), error = function(e) e)
  ...
  model_function <- function(data, formula) {
    ...
    R2 <- ((summary(model)$coefficients["x", "Estimate"])^2 * var(df$x)) / var(df$y)
    ...
    if (!sd_y0) {
      ...
      keep_preds <- predictors[vapply(predictors, function(p) sd(df[[p]], na.rm = TRUE) > 0, logical(1))]
      if (length(keep_preds) > 0) {
        for (p in keep_preds) df[[paste0(p, "_z")]] <- safe_scale(df[[p]])
    ...
    if (x_type == "numeric") {
      ...
      parameter <- c(coef = coef, lower_CI_coef = lower_CI_coef,
                     upper_CI_coef = upper_CI_coef, R2 = R2,
                     coef_std = coef_std, lower_CI_coef_std = lower_CI_coef_std,
                     upper_CI_coef_std  = upper_CI_coef_std,
                     pearson = pearson, lower_CI_pearson = lower_CI_pearson,
      ...
      parameter <- c(coef = coef, lower_CI_coef = lower_CI_coef,
                     ...
                     R2 = R2, coef_std = coef_std, lower_CI_coef_std = lower_CI_coef_std,
                     upper_CI_coef_std = upper_CI_coef_std, 
                     cohen = cohen, lower_CI_cohen = lower_CI_cohen,
  ...
  estimates_to_long <- function(estimates, grid) {
    ...
    long <- do.call(rbind, lapply(seq_along(estimates), function(i) {
      scen <- grid[i, , drop = FALSE]
      samples <- estimates[[i]]
      do.call(rbind, lapply(names(samples), function(sname) {
        v <- samples[[sname]]
        data.frame(
          beta1    = scen$beta1,
...
get_x_cols <- function(f_exp, df) {
  ...
  if (length(x_term_idx) != 1) return(character(0))

  x_cols <- colnames(mm)[asg == x_term_idx]
  setdiff(x_cols, "(Intercept)")
}

true_quantities <- function(beta1, n, dgp_fun, formula, B = 10000, seed = 58294) {
  ...
  for (b1 in beta1) {
    for (iter in seq_len(B)) {
      ...
      truth_rows[[it_truth]] <- list(
        ...
        var_x = var_x,
        var_y = var_y,
        m0 = m0, m1 = m1, sd0 = sd0, sd1 = sd1, n0 = n0, n1 = n1,
      ...
      for (rr in restrict_levels) {
        res_rr <- if (rr == "none") res_none else dgp_fun(beta1 = b1, restrict = rr, n = n)
  ...
  scalars_df <- do.call(rbind, lapply(truth_rows, function(z) {
    data.frame(
      beta1  = z$beta1,
      x_type = z$x_type,
      var_x  = z$var_x,
  ...
  true_tbl$pearson  <- true_tbl$coef * sqrt(true_tbl$var_x_mean) / sqrt(true_tbl$var_y_mean)
  true_tbl$coef_std <- NA_real_
  true_tbl$cohen    <- NA_real_
  
  for (b1 in unique(true_tbl$beta1)) {
    idx <- which(vapply(truth_rows, function(z) isTRUE(z$beta1 == b1), logical(1)))
    
    good <- idx[vapply(truth_rows[idx], function(z) {
      is.matrix(z$Sxx) && is.numeric(z$syy) && isTRUE(is.finite(z$syy)) &&
        is.character(z$x_col) && length(z$x_col) == 1 && !is.na(z$x_col)
    }, logical(1))]
    
    if (length(good) < 5) next
    
    cols0 <- truth_rows[[good[1]]]$Xcols
    if (!all(vapply(good, function(ii) identical(truth_rows[[ii]]$Xcols, cols0), logical(1)))) next
    
    xcol0 <- truth_rows[[good[1]]]$x_col
    if (!all(vapply(good, function(ii) identical(truth_rows[[ii]]$x_col, xcol0), logical(1)))) next
    
    Sxx_bar <- Reduce(`+`, lapply(good, function(ii) truth_rows[[ii]]$Sxx)) / length(good)
    sxy_bar <- Reduce(`+`, lapply(good, function(ii) truth_rows[[ii]]$sxy)) / length(good)
    syy_bar <- mean(vapply(good, function(ii) truth_rows[[ii]]$syy, numeric(1)), na.rm = TRUE)
    
    sdX <- sqrt(diag(Sxx_bar))
    sdY <- sqrt(syy_bar)
    if (any(!is.finite(sdX)) || any(sdX == 0) || !is.finite(sdY) || sdY == 0) next
    
    Rxx_bar <- Sxx_bar / (sdX %o% sdX)
    rxy_bar <- sxy_bar / (sdX * sdY)
    
    beta_std_all <- tryCatch(solve(Rxx_bar, rxy_bar), error = function(e) rep(NA_real_, length(rxy_bar)))
    names(beta_std_all) <- cols0
    
    true_tbl$coef_std[true_tbl$beta1 == b1] <- unname(beta_std_all[xcol0])
  }
  
  sp2 <- ((true_tbl$n1 - 1) * true_tbl$sd1_mean^2 + (true_tbl$n0 - 1) * true_tbl$sd0_mean^2) /
...
run_simulations <- function(niter, beta1, restrict, n, formula, dgp_fun) {
  ...
  data_joined <- data %>%
    ...
    left_join(
      data_varred %>%
        select(beta1, restrict, sample, target_var,
  ...
  data_relbias <- data_joined %>%
    ...
    select(
      beta1, restrict, sample, parameter, estimate, iter, x_type,
      true_value, abs_bias, abs_bias_pct, rel_bias, rel_bias_pct,
      target_var, true_variance, true_variance_none,
      variance_reduction, variance_reduction_pct
    )
...
simulation_results <- function(sim, restrict){
  ...
  if(unique(sim$x_type) == "binary"){
    keep_params <- c(
    "coef", "coef_std", "R2", "cohen")
  }else {
    keep_params <- c(
    "coef", "coef_std", "R2", "pearson")
  }
  ...
  plot_df <- summary_mc %>%
    mutate(mean = mean_rel_pct,
           se = mc_se_pct, 
           restrict = factor(restrict, levels = c("none", "weak","moderate","strong"))) %>%
  ...
  px_other <- plot_df %>%
    ...
    ggplot(aes(x = factor(beta1), y = mean, color = restrict, group = restrict)) +
    geom_hline(yintercept = 0, linetype = "dashed", color = "gray60") +
    geom_line(position = position_dodge(width = 0.6), linewidth = 0.5, show.legend = FALSE) +
  ...
  print(px_other)
  ggsave("Mean relative bias for convenience sample restricted on X.png", width = 10, height = 10)
  ...
  print(py_other)
  ggsave("Mean relative bias for convenience sample restricted on Y.png", width = 10, height = 10)
}
...
power <- function(sim) {
  ...
  power_tbl <- bind_rows(lapply(families, function(f) get_power_one_family(sim, f)))
  ...
  power_tbl_with_truth <- bind_rows(power_tbl, none_expanded) %>%
    mutate(restrict = factor(restrict, levels = c("none", "weak","moderate","strong")))
